...
async def get_latest(
    params: Optional[GetLatestFirmwareOptions] = None,
) -> LatestFirmware:
    """
    ...
    result = LatestFirmware(version=response["data"]["firmware"]["version"])

    if params.do_download:
        result.firmware = await download_file(
            response["data"]["firmware"]["downloadUrl"]
        )
