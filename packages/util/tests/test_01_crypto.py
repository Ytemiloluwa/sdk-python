...
class TestCrypto:
    ...
    def test_int_to_uint_byte(self):
        ...
        for test_case in test_cases:
            result = int_to_uint_byte(
                test_case["params"]["ele"], test_case["params"]["radix"]
            )
        ...
        for test_case in test_cases:
            with pytest.raises(Exception):
                pad_start(
                    ...
                    test_case["pad_string"],
                )

    def test_uint8array_to_hex(self):
        test_cases = [
            {"uint8": bytearray([]), "result": ""},
            {"uint8": bytearray([12, 255, 78]), "result": "0cff4e"},
            {"uint8": bytearray([213, 204, 204, 100, 217]), "result": "d5cccc64d9"},
            {
                "uint8": bytearray(
    ...
    def test_hex_to_uint8array(self):
        test_cases = [
            {"uint8": bytearray([]), "hex": ""},
            {"uint8": bytearray([12, 255, 78]), "hex": "0cff4e"},
            {"uint8": bytearray([12, 255, 78]), "hex": "cff4e"},
            {"uint8": bytearray([213, 204, 204, 100, 217]), "hex": "d5cccc64d9"},
            {
                "uint8": bytearray(
                    [12, 102, 56, 236, 103, 69, 123, 56, 48, 199, 71, 250, 121]
                ),
