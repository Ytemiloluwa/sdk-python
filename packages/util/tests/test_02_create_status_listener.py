...
class TestCreateStatusListener:
    def test_trigger_events(self):
        for test_case in fixtures["valid"]:
            ...
            assert force_status_update_func is not None, "forceStatusUpdate function should be defined"
            ...
            assert (
                on_event_for_this_subtest.call_count == len(expected_event_calls_from_fixture)
            ...
            for i, expected_single_call_args_list in enumerate(expected_event_calls_from_fixture):
                ...
                assert (
                    len(actual_args_as_tuple) == len(expected_single_call_args_list)
