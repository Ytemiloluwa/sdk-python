        ...
        def make_on_data(captured_test_case):
            async def on_data(data):
                ...
                if decoded_packet:
                    ...
                    print(
                        f"Received packet with sequence {seq_num}, generating CMD_ACK"
                    )
                    ...
                    if decoded_ack:
                        print(
                            f"Generated ACK packet type: {decoded_ack[0]['packet_type']}, sequence: {decoded_ack[0]['sequence_number']}"
                        )
                        print(
                            f"CMD_ACK constant value: {config_v3.commands.PACKET_TYPE.CMD_ACK}"
                        )

                    await connection.mock_device_send(ack_packet)
                    print(
                        f"ACK sent to pool. Pool size: {len(await connection.peek())}"
                    )
