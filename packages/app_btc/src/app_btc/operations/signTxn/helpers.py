...
def assert_sign_txn_params(params: SignTxnParams) -> None:
    ...
    for i, input_data in enumerate(params.txn.inputs):
        ...
        assert_condition(
            input_data.address, f"txn.inputs[{i}].address should be define"
        )
        assert_condition(
            input_data.change_index, f"txn.inputs[{i}].changeIndex should be define"
        )
        assert_condition(
        ...
        assert_condition(
            input_data.prev_index, f"txn.inputs[{i}].addressIndex should be define"
        )

        assert_condition(
            input_data.prev_txn_id, f"txn.inputs[{i}].prevTxnId should not be empty"
        )
        assert_condition(
