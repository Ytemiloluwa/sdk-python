...
from app_btc.operations.signTxn.types import (
    SignTxnParams,
    SignTxnTxnData,
    SignTxnInputData,
    SignTxnOutputData,
...
def create_sign_txn_params(params_dict):
    ...
    if not params_dict:
        return SignTxnParams(
            wallet_id=b"", derivation_path=[], txn=SignTxnTxnData(inputs=[], outputs=[])
        )
    ...
    if txn_data is None:
        txn_data = {}
    inputs = [
        SignTxnInputData(**input_data) for input_data in txn_data.get("inputs", [])
    ]
    outputs = [
        SignTxnOutputData(**output_data) for output_data in txn_data.get("outputs", [])
    ]
    ...
    async def test_should_be_able_to_sign_txn(self, btc_app: BtcApp):
        for test_case in fixtures.valid:
            ...
            if test_case.name == "With 1 input":
                ...
                assert (
                    output.signed_transaction == test_case.output["signed_transaction"]
                )
                ...
                try:
                    ...
                    assert (
                        ...
                        == test_case.output["signed_transaction"]
                    )
                    expect_mock_calls(test_case, on_event)
    ...
    async def test_should_throw_error_when_device_returns_error(self, btc_app: BtcApp):
        for test_case in fixtures.error:
            ...
            try:
                ...
                assert "Invalid result" in str(e) or "DeviceAppError" in str(
                    type(e).__name__
                )
    ...
    async def test_should_throw_error_when_device_returns_invalid_data(
        ...
        for test_case in fixtures.invalid_data:
            ...
            try:
                ...
                pytest.fail(
                    f"Expected {test_case.error_instance} but got {type(unexpected_error)}: {unexpected_error}"
                )
