...
with_unknown_error = GetPublicKeyTestCase(
    ...
    results=[
        ResultData(
            name="error",
            data=Result(common_error=CommonError(unknown_error=1)).SerializeToString(),
        )
    ],
...
with_invalid_app_id = GetPublicKeyTestCase(
    ...
    params=common_params["params"],
    queries=common_params["queries"],
    results=[ResultData(name="error", data=Result().SerializeToString())],
    error_instance=DeviceAppError,
    error_message=deviceAppErrorTypeDetails[DeviceAppErrorType.INVALID_MSG_FROM_DEVICE][
        "message"
    ],
)
...
with_card_error = GetPublicKeyTestCase(
    ...
    error_instance=DeviceAppError,
    error_message=deviceAppErrorTypeDetails[DeviceAppErrorType.CARD_OPERATION_FAILED][
        "message"
    ],
)
